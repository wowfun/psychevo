...
fn tui_snapshot_model_bottom_panel() {
    let temp = tempdir().expect("temp");
    let app = test_app_with_models(&temp);
    let mut ui = fixture_ui(&app, FixtureKind::Idle);
...
fn tui_snapshot_variant_bottom_panel() {
    ...
    let other = models
        ...
        .find_map(|row| match &row.value {
            BottomSelectionValue::Model(model) if model.model == "other-model" => {
                Some(model.clone())
            }
            _ => None,
        })
        .expect("other model");
    let mut ui = fixture_ui(&app, FixtureKind::Idle);
    ui.bottom_panel = Some(app.variant_panel(other, models));
...
async fn model_command_opens_searchable_bottom_picker() {
    ...
    assert_eq!(panel.rows[filtered[0]].label, "mock/other-model");
}
