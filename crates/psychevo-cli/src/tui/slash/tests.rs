use super::{
    SlashCommand, SlashMenuItem, SlashShortcutMatch, TuiExportOptions, TuiShareOptions,
    base_slash_menu_items, configured_slash_menu_items, format_slash_help,
    format_slash_help_with_config, parse_effective_slash_config, parse_slash_command,
    parse_slash_command_with_config, slash_help_sections_with_config, slash_menu_items,
    slash_menu_items_from, slash_prefix_menu_items,
};
...
fn parses_export_share_commands() {
    ...
    assert_eq!(
        ...
        Some(SlashCommand::Export(TuiExportOptions {
            path: Some("out.json".to_string()),
            format: SessionExportFormat::Json,
            include: SessionExportIncludeSet::parse("h,lpr", SessionArtifactKind::Export).unwrap(),
        }))
    );
    assert_eq!(
    ...
    assert_eq!(
        parse_slash_command("/share share.md --include messages,reasoning,provider-input-evidence")
            .unwrap(),
    ...
    assert_eq!(
        ...
        Some(SlashCommand::Share(TuiShareOptions {
            path: Some("share.md".to_string()),
            include: SessionExportIncludeSet::parse("m,r,pie", SessionArtifactKind::Share).unwrap(),
        }))
    );
...
fn formats_slash_help_from_registry() {
    ...
    assert!(help.contains("/<skill-or-bundle> [args] - submit a skill or bundle (2 available)"));
    assert!(help.contains(
