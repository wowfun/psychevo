use super::{
    FakeAsrProvider, FakeRealtimeProvider, FakeTtsProvider, VoiceAsrProvider, VoiceAsrRequest,
    VoiceAudioFormat, VoiceAudioInput, VoiceRealtimeEvent, VoiceRealtimeProvider,
    VoiceRealtimeStartRequest, VoiceRealtimeTransport, VoiceTtsProvider, VoiceTtsRequest,
    parse_xiaomi_asr_response, parse_xiaomi_tts_response, parse_xiaomi_tts_sse_audio_delta,
    validate_voice_asr_request, xiaomi_asr_request_body, xiaomi_tts_request_body,
};
...
async fn fake_realtime_emits_started_transcript_and_closed() {
    ...
    let (result, mut events) = provider
        .start(
            VoiceRealtimeStartRequest {
                ...
                transport: VoiceRealtimeTransport::Websocket,
                voice: None,
                sdp_offer: None,
            },
            AbortSignal::new(rx),
    ...
    assert!(matches!(started, VoiceRealtimeEvent::Started { .. }));
    ...
    let closed = futures::StreamExt::next(&mut events)
        ...
        .expect("closed")
        .expect("closed event");
    assert!(matches!(closed, VoiceRealtimeEvent::Closed { .. }));
}
