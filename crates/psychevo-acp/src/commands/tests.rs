...
use agent_client_protocol::schema::v2::{
    DiffChangeOperation, SessionUpdate, ToolCallContent, ToolCallStatus, ToolKind,
};
...
use psychevo::session_export::{SessionArtifactKind, SessionExportFormat, SessionExportInclude};
use psychevo::workspace_diff::{
    WorkspaceDiff, WorkspaceDiffFile, WorkspaceDiffFileStatus, WorkspaceDiffTruncation,
};
use serde_json::Value;

use super::slash_diff_sessions::{diff_tool_call_updates, parse_artifact_args};
use super::{ACP_COMMAND_ADVERTISEMENT_LIMIT, acp_command_capabilities};
...
fn acp_export_parses_last_provider_response_include() {
    ...
    assert!(
        parsed.include.is_some_and(|include| {
    ...
    assert!(share.is_err());
    assert!(
        parse_artifact_args("out.json -i last-raw-response", SessionArtifactKind::Export).is_err()
    );
}
