...
pub(crate) async fn sqlite_schema_v32_creates_framework_and_gateway_coordination_schema() {
    ...
    assert!(
        sqlite_columns(&conn, "framework_interactions")
            .iter()
            .any(|name| name == "interaction_id")
    );
    assert!(
        sqlite_columns(&conn, "gateway_source_bindings")
            .iter()
...
pub(crate) async fn framework_interaction_writes_are_store_first_and_turn_scoped() {
    ...
    let cwd = canonical_cwd(&temp.path().join("work")).expect("cwd");
    let thread_id = store
        .create_session_with_metadata(&cwd, "sdk", "model", "provider", None)
        .await
        .expect("thread");
    ...
    let reused = store
        .framework_interactions_for_thread(&thread_id, false)
        .await
        .expect("reused interactions")
        .into_iter()
        .filter(|record| record.interaction_id == "call_1")
        .collect::<Vec<_>>();
