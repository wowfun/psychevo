use super::super::specs::{
    CommandCapability, CommandDestination, CommandPresentationKind, CommandSurface,
    DynamicSlashCommand, SLASH_COMMANDS, SlashCommandAction, SlashCommandEffect, SlashCommandParse,
    SlashCommandSurface,
};
use super::{
    CLI_COMMANDS, available_slash_commands_for_surface, parse_slash_command_line,
...
fn cli_registry_records_process_commands() {
    ...
    assert_eq!(
        names,
        [
            "init", "run", "tui", "serve", "gateway", "acp", "mcp", "profile", "agent", "skill",
...
fn undo_redo_require_session_revert_capability() {
    ...
    let with_revert =
        available_slash_commands_for_surface(&[CommandCapability::SessionRevert], false, &[], 100);
...
fn shared_effect_parses_btw_when_side_conversations_are_supported() {
    let SlashCommandParse::Known(invocation) = parse_slash_command_line("/btw explain this") else {
        panic!("expected known command");
    };
    let effect = slash_invocation_effect(
        &invocation,
...
fn shared_effect_parses_mission_team_and_goal() {
    ...
    let effect = slash_invocation_effect(&invocation, &[], SlashCommandSurface::WebDesktop, false)
...
fn shared_effect_parses_voice_modes() {
    ...
    let effect = slash_invocation_effect(&invocation, &[], SlashCommandSurface::Messaging, true)
        .expect("voice effect");
...
fn web_desktop_surface_returns_surface_specific_guidance() {
    let SlashCommandParse::Known(invocation) = parse_slash_command_line("/image ./a.png") else {
        panic!("expected known command");
    };
    let effect = slash_invocation_effect(&invocation, &[], SlashCommandSurface::WebDesktop, false)
...
fn shared_effect_parses_text_forms() {
    let SlashCommandParse::Known(invocation) = parse_slash_command_line("/tools enable web") else {
        panic!("expected known command");
    };
    let effect = slash_invocation_effect(
        &invocation,
