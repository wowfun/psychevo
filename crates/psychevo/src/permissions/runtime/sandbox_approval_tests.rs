...
impl crate::types::ApprovalHandler for SwappingApprovalHandler {
    ...
    fn request_permission(
        ...
        std::os::unix::fs::symlink(&self.replacement, &self.link).expect("replace symlink");
...
fn sandbox_policy(cwd: &Path, mode: crate::sandbox::SandboxMode) -> crate::sandbox::SandboxPolicy {
...
fn permission_runtime(
    ...
    permission_runtime_with_config(cwd, PermissionConfig::default(), policy, grants, handler)
}
...
fn wrapped_write(
    ...
    runtime
        .wrap_tools(vec![Arc::new(crate::tools::write::WriteTool::new(
            cwd.to_path_buf(),
...
fn wrapped_edit(
    ...
    runtime
        .wrap_tools(vec![Arc::new(crate::tools::edit::EditTool::new(
            cwd.to_path_buf(),
            tool_context(policy, grants),
        )) as Arc<dyn ToolBinding>])
        .into_iter()
...
async fn workspace_write_through_symlink_to_external_requires_approval() {
    ...
    let runtime = permission_runtime(work.path(), policy.clone(), grants.clone(), handler.clone());
    ...
    assert_eq!(
        ...
        "linked-outside/nested/escaped.txt"
    );
    assert_eq!(
...
fn active_permission_config_remains_protected_through_a_symlink() {
    let work = tempfile::tempdir().expect("work");
    let outside = tempfile::tempdir().expect("outside");
    std::os::unix::fs::symlink(outside.path(), work.path().join(".psychevo")).expect("symlink");
    let runtime = PermissionRuntime::new(
        work.path().to_path_buf(),
        work.path().join(".psychevo"),
        PermissionConfig::default(),
...
async fn allow_once_grants_current_write_call_only() {
    ...
    let grants = crate::sandbox::SandboxWriteGrants::default();
    let handler =
        RecordingApprovalHandler::new(vec![crate::types::PermissionApprovalDecision::allow_once()]);
    let runtime = permission_runtime(work.path(), policy.clone(), grants.clone(), handler.clone());
    let write = wrapped_write(work.path(), policy, grants, &runtime);
