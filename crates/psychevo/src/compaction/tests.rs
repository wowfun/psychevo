...
use psychevo_ai::{
    AdapterCall, AdapterFuture, AdapterStream, DeploymentConfig, FinishReason, FinishReasonKind,
    GenerationOutcome, LanguageAdapter, LanguageAdapterEvent, LanguageRequest, Outcome, Provider,
};
...
async fn rolling_summary_failure_returns_before_any_checkpoint_write() {
    ...
    assert!(
        error
            .to_string()
            .contains("synthetic rolling summary failure")
    );
...
"#,
    ...
    assert!(
        !auto_compaction_due_for_snapshot(&options, &snapshot(90_000, None)).expect("unbounded")
    );
}
