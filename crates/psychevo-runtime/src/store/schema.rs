...
mod tests {
    ...
    #[tokio::test]
    async fn concurrent_first_open_records_one_v28_migration_history() {
    ...
    #[tokio::test]
    async fn existing_v28_schema_registers_baseline_without_rewriting_data() {
        let temp = tempfile::tempdir().expect("tempdir");
        ...
        let reopened = StateRuntime::open(&db_path)
            .await
            .expect("register existing v28 schema");
        assert!(
            reopened
    ...
    #[tokio::test]
    async fn successful_writes_publish_diagnostics_and_trigger_passive_checkpoint() {
        let temp = tempfile::tempdir().expect("tempdir");
