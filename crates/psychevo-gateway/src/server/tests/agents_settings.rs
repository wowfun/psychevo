...
"#,
    ...
    let project_config = std::fs::read_to_string(state.inner.workdir.join(".psychevo/config.toml"))
        .expect("project config");
...
"#;
    ...
    assert_eq!(
        std::fs::read_to_string(state.inner.workdir.join(".psychevo/config.toml"))
            .expect("project config"),
        project_config
    );
}
