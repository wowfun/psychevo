...
use psychevo_gateway_protocol::events_transcript::{
    TranscriptBlockKind, TranscriptBlockStatus, TranscriptEntryRole,
};

use super::{
    TurnProjectionWindow, agent_relationship_lookup_candidates,
...
#[test]
fn compaction_projection_preserves_framework_boundary_and_wire_metadata() {
    let entries = project_compaction_entries(
        "thread-1",
        &[ThreadCompaction {
            checkpoint_id: 7,
            created_at_ms: 20,
            ...
            first_kept_session_seq: 11,
            boundary_session_seq: 10,
            tokens_before: Some(200),
            tokens_after: Some(80),
            summary_provider: "provider".to_string(),
            summary_model: "model".to_string(),
            instructions: Some("retain decisions".to_string()),
            metadata: Some(json!({"source": "test"})),
        }],
    );

    assert_eq!(entries[0].id, "compaction:7");
    assert_eq!(entries[0].thread_id, "thread-1");
    assert_eq!(
        entries[0].metadata.as_ref().unwrap()["created_after_session_seq"],
        10
    );
    assert_eq!(
        entries[0].metadata.as_ref().unwrap()["checkpoint"]["source"],
        "test"
    );
    assert_eq!(
        entries[0].blocks[0].preview.as_deref(),
        Some("auto | 200 -> 80 tokens | keeps from #11")
    );
}
...
fn terminal_projection_keeps_failed_and_interrupted_turns_visible() {
    let entries = project_turn_terminal_entries(
        ...
        &[
            ThreadTurnTerminal {
                ...
                first_committed_session_seq: None,
                boundary_session_seq: i64::MAX,
                completed_at_ms: 4,
                metadata: Some(json!({"source": "test"})),
            },
            ThreadTurnTerminal {
                turn_id: "turn-interrupted".to_string(),
                status: ThreadTurnTerminalStatus::Interrupted,
    ...
    assert_eq!(
        entries
            ...
            .collect::<Vec<_>>(),
        vec![
            "turn:turn-failed:terminal",
            "turn:turn-interrupted:terminal"
        ]
    );
...
fn terminal_reconciliation_marks_yielded_exec_block_failed_after_turn_failure() {
    ...
    let terminal = ThreadTurnTerminal {
        turn_id: "turn-failed".to_string(),
        status: ThreadTurnTerminalStatus::Failed,
        outcome: Some("failed".to_string()),
        error_message: Some("provider failed".to_string()),
        first_committed_session_seq: Some(2),
        boundary_session_seq: 1,
        completed_at_ms: 4,
        metadata: Some(json!({
            "source": "gateway",
            "firstCommittedSeq": 2
    ...
    reconcile_terminal_bounded_running_blocks(&mut entries, "thread-1", &[terminal]);
...
fn projector_preserves_hosted_web_lifecycle_and_clickable_sources() {
    let entries = project_transcript_entries(
        ...
        &[summary(
            ...
            Message::Assistant {
                content: vec![
                    AssistantBlock::ProviderTool(psychevo::application::ProviderToolBlock {
                        id: "ws_1".into(),
                        name: "web_search".into(),
                        action: Some(json!({"type":"search","query":"rust news"})),
                        status: "completed".into(),
                    }),
                    AssistantBlock::Source {
                        source: psychevo::application::AssistantSource::UrlCitation(
                            psychevo::application::UrlCitationSource {
    ...
    assert_eq!(
        entries[0].blocks[0].title.as_deref(),
        Some("Searched the web")
    );
    assert_eq!(
        entries[0].blocks[1].metadata.as_ref().unwrap()["projection"],
        "url_citation"
    ...
    assert_eq!(
        entries[0].blocks[1].metadata.as_ref().unwrap()["url"],
...
fn projector_decodes_wrapped_local_web_search_result_for_committed_transcript() {
    ...
    let public_result: Value =
        serde_json::from_str(&block.result.as_ref().expect("tool result").content)
...
fn projector_materializes_acp_plan_metadata_as_a_display_only_status_block() {
    ...
    assert_eq!(
        plan.metadata.as_ref().unwrap()["projection"],
        "acp_peer_plan"
    );
...
fn projector_reloads_agent_tool_calls_as_agent_blocks() {
    ...
    assert_eq!(
        block.metadata.as_ref().unwrap()["result"]["child_session_id"],
        "child-thread"
    );
...
#[test]
fn agent_relationship_enrichment_restores_committed_child_session_target() {
    let summaries = vec![
        summary(
            1,
            Message::Assistant {
    ...
    assert_eq!(
        agent_relationship_lookup_candidates(&entries),
        vec![
    ...
    let mut relationship = agent_relationship(
        "parent-thread",
        "child-thread",
        "agent-run-1",
        "translate",
    ...
    let agent = relationship.agent.as_mut().expect("Agent relationship");
    agent.description = Some("Translate messages".to_string());
    agent.team_run_id = Some("team-run-1".to_string());
    agent.mission_run_id = Some("mission-run-1".to_string());
    agent.team_name = Some("localization".to_string());
    agent.team_member_id = Some("translator".to_string());
    agent.runtime_ref = Some("acp/translator".to_string());
    relationship.status = AgentRelationshipStatus::Open;
    let relationships = vec![relationship];

    enrich_agent_blocks_from_relationships(&mut entries, &relationships);
    ...
    assert_eq!(metadata["result"]["task_name"], "zh-to-en");
    assert_eq!(metadata["result"]["task"], "Translate 你好 to English");
    assert_eq!(
        metadata["result"]["agent_description"],
        "Translate messages"
    );
    assert_eq!(metadata["result"]["parent_tool_call_id"], "call_agent_zh");
    assert_eq!(metadata["result"]["team_run_id"], "team-run-1");
    assert_eq!(metadata["result"]["mission_run_id"], "mission-run-1");
    assert_eq!(metadata["result"]["team_name"], "localization");
    assert_eq!(metadata["result"]["team_member_id"], "translator");
    assert_eq!(metadata["result"]["runtime_ref"], "acp/translator");
    assert_eq!(
        entries[0].blocks[0]
            .result
            .as_ref()
...
#[test]
fn agent_relationship_enrichment_does_not_make_failed_agent_blocks_openable() {
    let summaries = vec![
        summary(
            1,
            Message::Assistant {
    ...
    let mut entries = project_transcript_entries("parent-thread", &summaries);
    let relationships = vec![agent_relationship(
        "parent-thread",
        "child-thread",
        "agent-run-1",
        "translate",
...
fn projector_keeps_selected_skill_metadata_on_user_entry() {
    ...
    assert_eq!(
        entries[0].blocks[0].metadata.as_ref().unwrap()["prompt_prefix"]["selected_skills"][0]["path"],
        "/tmp/x/SKILL.md"
    );
}
...
fn summary(session_seq: i64, message: Message) -> ThreadItem {
    ThreadItem {
        session_seq,
        message,
        usage: None,
