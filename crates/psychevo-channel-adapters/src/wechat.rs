...
pub async fn fetch_wechat_qr_code(
    ...
    let qrcode = body
        ...
        .ok_or_else(|| anyhow!("WeChat QR response did not include qrcode"))?;
    let qr_url = body
        .qrcode_img_content
        .filter(|value| !value.trim().is_empty())
    ...
    match status.as_str() {
        "confirmed" => {
            ...
            if account_id.is_empty() || token.is_empty() {
                return Err(anyhow!(
                    "WeChat QR confirmation did not include account id and token"
                ));
            }
...
pub fn render_wechat_qr_svg(value: &str) -> Result<String> {
    let code = qrcode::QrCode::new(value.as_bytes())
        .map_err(|err| anyhow!("failed to render WeChat QR code: {err}"))?;
    Ok(code
...
pub fn render_wechat_qr_terminal(value: &str) -> Result<String> {
    let code = qrcode::QrCode::new(value.as_bytes())
        .map_err(|err| anyhow!("failed to render WeChat QR code: {err}"))?;
    Ok(code
...
impl WechatIlinkAdapter {
    pub fn new(config: WechatIlinkConfig) -> Result<Self> {
        if config.token.trim().is_empty() {
            return Err(anyhow!("WeChat iLink adapter requires a bot token"));
        }
    ...
    fn poll(&self) -> BoxFuture<'static, Result<Vec<ImInboundMessage>>> {
        ...
        Box::pin(async move {
            ...
            if wechat_ilink_session_expired(&body) {
                return Err(anyhow!(wechat_expired_session_message(&body)));
            }
    ...
    fn send(&self, message: ImOutboundMessage) -> BoxFuture<'static, Result<()>> {
        ...
        Box::pin(async move {
            if message.text.trim().is_empty() {
                return Err(anyhow!("WeChat iLink outbound text cannot be empty"));
            }
...
fn wechat_headers(token: &str, body: &str) -> Result<HeaderMap> {
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&format!("Bearer {token}"))
            .map_err(|err| anyhow!("invalid WeChat token header: {err}"))?,
    );
    headers.insert(
        CONTENT_LENGTH,
        HeaderValue::from_str(&body.len().to_string())
            .map_err(|err| anyhow!("invalid WeChat content length: {err}"))?,
    );
    headers.insert(
        "X-WECHAT-UIN",
        HeaderValue::from_str(&BASE64_STANDARD.encode(now_millis().to_string()))
            .map_err(|err| anyhow!("invalid WeChat uin header: {err}"))?,
    );
    headers.insert(
        "iLink-App-Id",
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&WECHAT_ILINK_CLIENT_VERSION.to_string())
            .map_err(|err| anyhow!("invalid WeChat client version header: {err}"))?,
    );
    Ok(headers)
}
