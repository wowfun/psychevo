...
impl FeishuLarkLongConnectionAdapter {
    pub fn new(config: FeishuLarkLongConnectionConfig) -> Result<Self> {
        if config.app_id.trim().is_empty() || config.app_secret.trim().is_empty() {
            return Err(anyhow!(
                "Feishu/Lark adapter requires app id and app secret"
            ));
        }
        let base_url = config
            .base_url
    ...
    async fn stop_long_connection(&self) -> Result<()> {
        ...
        match task.await {
            ...
            Err(err) => Err(anyhow!("Feishu/Lark stream task failed: {err}")),
        }
    }
}
    ...
    fn send(&self, message: ImOutboundMessage) -> BoxFuture<'static, Result<()>> {
        ...
        Box::pin(async move {
            if message.text.trim().is_empty() {
                return Err(anyhow!("Feishu/Lark outbound text cannot be empty"));
            }
...
fn feishu_error(err: feishu_sdk::core::Error) -> Error {
    anyhow!("Feishu/Lark adapter failed: {err}")
}
