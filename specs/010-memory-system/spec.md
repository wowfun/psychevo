...
Define the optional memory system boundary for reusable knowledge across runs or sessions.
...
- evidence relationship for memory candidates and mutations that arise from a run
...
A memory system is an optional runtime-adjacent mechanism for retaining reusable knowledge across runs or sessions.

Execution, durable evidence, and session continuity must work without memory. Memory may improve later runs, but it must not become required execution substrate.
...
Memory candidates should remain compact enough to be useful across future runs. This spec does not define selection criteria, quality scoring, freshness, priority, or trust policy.

Memory candidates that arise from a run should be evidence-linked. Evidence links preserve where the candidate came from; they do not make memory part of the execution record.
...
This spec does not define storage, retrieval, ranking, provider routing, background refresh, synchronization, or capacity management.
...
Memory mutations that arise during a run should be observable. This spec does not define mutation events, records, storage commits, conflict handling, undo, approval behavior, or user-facing output.

## Evidence Relationship

Memory candidates and memory mutations that arise from a run should be linkable to durable evidence.
...
- [004 Runtime Contract](../004-runtime-contract/spec.md) defines runtime run assembly without requiring memory.
