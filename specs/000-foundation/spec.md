...
- Execution leaves evidence. Agent behavior should produce explicit records that make sessions, agent invocations, messages, tool activity, and outcomes inspectable and replay-ready. Important behavior must not depend on hidden state that cannot be reconstructed from durable evidence.
...
- [050 Capability Extensions](../050-capability-extensions/spec.md) defines how extension declarations remain outside the minimal core until host-owned runtime acceptance.
