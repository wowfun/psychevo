...
session 会成为报告中的一个 Trial。报告先显示 report notes、可切换指标的
Visible Heatmap、Leaderboard，然后显示选中的 Trial trajectory。对比表刻意不展示
benchmark 和 task 列。热力图中每个 session 占一行，左侧是 session/trial 标签，
右侧是指标单元格，因此大量 session 会纵向增长，而不是形成很长的横向条带。
...
elapsed time 和 tool execution time 分别按选中 Trial 内的同类时间缩放，因此它只是
视觉提示，不是新的报告指标。
